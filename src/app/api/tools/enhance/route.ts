...
export const POST = withAuth(async (request: NextRequest, { userId }) => {
  try {
    ...
    // Check credits (3 credits for enhancement)
    ...
    if (!isAdmin) {
      ...
      // Deduct credits
      await CreditsService.deductCredits(userId, creditsRequired, 'enhance_quality')
    } else {
      console.log('Admin user - skipping credit deduction')
    }

    // Process image
    console.log('✨ Enhancing image for user:', userId)
    const processedImage = await ImageProcessingService.enhanceQuality(image)
